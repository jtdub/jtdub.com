...
Link State Advertisement (LSA) Types have never been my strong suit. I made a visual representation of how they are forwarded to help me get a better grasp on them.
...
* Type 4 - ASBR Summary - This is generated by an ABR informing other areas that an Autonomous System Border Router (ASBR) exists within its network.
...
When a type 5 LSA enters a stubby area, it's entered into the RIB as a default route, stripping the type 5 LSA routes. Type 5 LSAs are also not forwarded to not-so-stubby areas. To get external routes *INTO* an not-so-stubby area, the area has to be converted to a totally not-so-stubby-area, which will remove all summaries and inject a default route.
