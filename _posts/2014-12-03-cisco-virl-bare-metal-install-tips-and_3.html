...
In my first post on VIRL -  "
<a href="http://www.packetgeek.net/2014/12/cisco-virl-status-digital-paper-weight/" target="_blank">
 Cisco VIRL Status? Digital Paper Weight
</a>
", I shared my frustration with not being able to get my system to activate with Cisco. Come to find out, I'm a dim wit. That is, in the 'Salt ID and domain' section, I mistakenly left the
<em>
 .pem
</em>
suffix in the name. Ironically, I blurred out that section, in an attempt to retain some privacy. However, if I hadn't somebody may have rightly pointed out my error.
<br/>
<br/>
...
<br/>
<pre class="lang:default decode:true">auto p3p1<br/>iface p3p1 inet static<br/>     address 172.16.1.51<br/>     netmask 255.255.255.192<br/>     gateway 172.16.1.1</pre>
<br/>
...
<ul>
 ...
 <br/>
 <li>
  From here, the activation should work properly. Don't be a dim wit, like myself, and leave the
  <em>
   .pem
  </em>
  suffix on the Salt ID and domain.
 </li>
 <br/>
</ul>
<br/>
...
<br/>
I also noticed that the 'dummy' interfaces don't work above the number 4 - I attempted to create a dummy interface scheme that matched my ip prefixes. That didn't work out to well. I'm also noticed that the
<em>
 internalnet
</em>
section changes itself to 172.16.10.0/24 - according to /etc/network/interfaces and MySQL, but it's completely different in my /etc/virl.ini and I'm not certain where it's getting that prefix from, yet.
<br/>
<br/>
