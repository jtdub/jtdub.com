...
<br/>
<br/>
<pre>[root@sgnhv ~]# lvs<br/>  LV         VG       Attr     LSize   Pool Origin Data%  Move Log Copy%  Convert<br/>  lv_data    vg_sgnhv -wi-ao-- 500.00g                                                                                      <br/>  lv_root    vg_sgnhv -wi-ao--  50.00g                                           <br/>  lv_swap    vg_sgnhv -wi-ao--   5.88g                                           <br/><br/>[root@sgnhv ~]# lvcreate -n lv_crypto -L 1G vg_sgnhv<br/>  Logical volume "lv_crypto" created<br/><br/>[root@sgnhv ~]# lvs<br/>  LV         VG       Attr     LSize   Pool Origin Data%  Move Log Copy%  Convert<br/> <span style="background-color: yellow;"> lv_crypto  vg_sgnhv -wi-a---   1.00g </span>                                          <br/>  lv_data    vg_sgnhv -wi-ao-- 500.00g                                                                                      <br/>  lv_root    vg_sgnhv -wi-ao--  50.00g                                           <br/>  lv_swap    vg_sgnhv -wi-ao--   5.88g                                           <br/></pre>
<br/>
...
<br/>
<br/>
<pre>[root@sgnhv ~]# <span style="background-color: lime;">cryptsetup luksFormat /dev/vg_sgnhv/lv_crypto</span><br/><br/>WARNING!<br/>========<br/>This will overwrite data on /dev/vg_sgnhv/lv_crypto irrevocably.<br/><br/>Are you sure? (Type uppercase yes): YES<br/>Enter LUKS passphrase: <br/>Verify passphrase:</pre>
<br/>
...
<br/>
<br/>
<pre>[root@sgnhv ~]# ls /dev/mapper/<br/>control             vg_sgnhv-lv_data  vg_sgnhv-lv_libvirt  vg_sgnhv-lv_swap<br/><span style="background-color: yellow;">vg_sgnhv-lv_crypto</span>  vg_sgnhv-lv_home  vg_sgnhv-lv_root</pre>
<pre>[root@sgnhv ~]# <span style="background-color: lime;">cryptsetup luksOpen /dev/vg_sgnhv/lv_crypto crypt_dev_mapper</span><br/>Enter passphrase for /dev/vg_sgnhv/lv_crypto: <br/></pre>
<br/>
...
<br/>
<br/>
<pre>[root@sgnhv ~]# ls /dev/mapper/<br/>control           <span style="background-color: yellow;">vg_sgnhv-lv_crypto</span>  vg_sgnhv-lv_home     vg_sgnhv-lv_root<br/><span style="background-color: yellow;">crypt_dev_mapper</span>  vg_sgnhv-lv_data    vg_sgnhv-lv_libvirt  vg_sgnhv-lv_swap</pre>
<br/>
...
<br/>
<br/>
<pre>[root@sgnhv ~]# mkfs -t ext4 /dev/mapper/crypt_dev_mapper <br/>mke2fs 1.41.12 (17-May-2010)<br/>Filesystem label=<br/>OS type: Linux<br/>Block size=4096 (log=2)<br/>Fragment size=4096 (log=2)<br/>Stride=0 blocks, Stripe width=0 blocks<br/>65408 inodes, 261632 blocks<br/>13081 blocks (5.00%) reserved for the super user<br/>First data block=0<br/>Maximum filesystem blocks=268435456<br/>8 block groups<br/>32768 blocks per group, 32768 fragments per group<br/>8176 inodes per group<br/>Superblock backups stored on blocks: <br/> 32768, 98304, 163840, 229376<br/><br/>Writing inode tables: done                            <br/>Creating journal (4096 blocks): done<br/>Writing superblocks and filesystem accounting information: done<br/><br/>This filesystem will be automatically checked every 26 mounts or<br/>180 days, whichever comes first.  Use tune2fs -c or -i to override.</pre>
<br/>
...
<br/>
<br/>
<pre>[root@sgnhv ~]# <span style="background-color: lime;">cryptsetup luksClose crypt_dev_mapper</span><br/></pre>
<br/>
...
<br/>
<br/>
<pre>[root@sgnhv ~]# vim /etc/crypttab <br/>[root@sgnhv ~]# cat /etc/crypttab <br/>crypt_dev_mapper /dev/vg_sgnhv/lv_crypto<br/></pre>
<br/>
...
<br/>
<br/>
<pre>[root@sgnhv ~]# <span style="background-color: lime;">dd if=/dev/urandom of=/etc/crypt_dev_mapper.key bs=1k count=4</span><br/>[root@sgnhv ~]# <span style="background-color: lime;">cryptsetup luksAddKey /dev/vg_sgnhv/lv_crypto /etc/crypt_dev_mapper.key</span> <br/>Enter any passphrase: <br/></pre>
<br/>
...
<br/>
<br/>
<pre>[root@sgnhv ~]# vim /etc/crypttab <br/>[root@sgnhv ~]# cat /etc/crypttab <br/>crypt_dev_mapper /dev/vg_sgnhv/lv_crypto /etc/crypt_dev_mapper.key<br/>[root@sgnhv ~]# chmod 400 /etc/crypt_dev_mapper.key <br/></pre>
<br/>
Once that is setup, you can modify your /etc/fstab to have the file system mounted at boot.
<br/>
<br/>
<pre>[root@sgnhv ~]# vim /etc/fstab <br/>[root@sgnhv ~]# tail -1 /etc/fstab <br/>/dev/mapper/crypt_dev_mapper /crypt  ext4 defaults 1 0<br/></pre>
<br/>
You can test the functionality out, without rebooting by doing the following:
<br/>
<br/>
<pre>[root@sgnhv ~]# cryptsetup luksClose /dev/mapper/crypt_dev_mapper <br/>[root@sgnhv ~]# bash<br/>[root@sgnhv ~]# . /etc/init.d/functions <br/>[root@sgnhv ~]# init_crypto 1<br/>[root@sgnhv ~]# mount -a                                   [  OK  ]<br/>[root@sgnhv ~]# df -h | grep crypt<br/>/dev/mapper/crypt_dev_mapper<br/>                     1006M   18M  938M   2% /crypt<br/></pre>
<br/>
...
<br/>
<br/>
For my personal preference, encrypting a notebooks entire filesystem or even a tablet or smart phone should be the course of action. In Linux, that can be done during the install. Otherwise, I'd propose something like
<a href="http://www.truecrypt.org/" target="_blank">
 TruCrypt
</a>
. Other than that, encrypting thumb drives would be handy.
